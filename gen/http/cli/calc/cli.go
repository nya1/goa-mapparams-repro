...
func UsageExamples() string {
	return os.Args[0] + ` calc multiply --type 1595097090304680186 --metadata '{
      "testKey": "val"
   }'` + "\n" +
		""
}
...
func ParseEndpoint(
	...
	var (
		...
		calcMultiplyTypeFlag     = calcMultiplyFlags.String("type", "", "")
		calcMultiplyMetadataFlag = calcMultiplyFlags.String("metadata", "", "")
	)
	...
	)
	{
		...
		case "calc":
			...
			case "multiply":
				endpoint = c.Multiply()
				data, err = calcc.BuildMultiplyPayload(*calcMultiplyTypeFlag, *calcMultiplyMetadataFlag)
			}
		}
	}
	if err != nil {
...
func calcMultiplyUsage() {
	fmt.Fprintf(os.Stderr, `%[1]s [flags] calc multiply -type INT -metadata JSON
...
Example:
    %[1]s calc multiply --type 1595097090304680186 --metadata '{
      "testKey": "val"
   }'
`, os.Args[0])
}
